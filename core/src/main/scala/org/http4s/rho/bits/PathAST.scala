...
object PathAST {
  ...
  case class TypedPath[F[_], M[_], T <: HList](rule: PathRule) extends UriConvertible[F] {

    /** Match this rule and then `next` rule
      *
      * Alias of `&&`
      */
    def and[T2 <: HList](next: TypedPath[F, M, T2])(implicit
        prep: Prepend[T2, T]): TypedPath[F, M, prep.Out] =
    ...
    /** Match this rule and then `next` rule
      ...
      * Alias of `and`
      */
    def &&[T2 <: HList](p2: TypedPath[F, M, T2])(implicit
        prep: Prepend[T2, T]): TypedPath[F, M, prep.Out] = and(p2)

    /** Match this rule or `alt` rule if this rule fails
      *
      * Alias of `||`
      */
    def or(alt: TypedPath[F, M, T]): TypedPath[F, M, T] = TypedPath(PathOr(this.rule, alt.rule))

    /** Match this rule or `alt` rule if this rule fails
      *
      * Alias of `or`
      */
    def ||(alt: TypedPath[F, M, T]): TypedPath[F, M, T] = or(alt)
    ...
    /** Match against a `String`
      ...
      */
    def /(segment: String): TypedPath[F, M, T] = TypedPath(PathAnd(this.rule, PathMatch(segment)))
    ...
    /** Capture a `String` from the path
      ...
      */
    def /(symbol: Symbol)(implicit model: M[String]): TypedPath[F, M, String :: T] = {
      val capture = PathCapture(symbol.name, None, StringParser.strParser[F, M], model)
    ...
    def /[T2 <: HList](builder: RequestLineBuilder[F, M, T2])(implicit
        prep: Prepend[T, T2]): RequestLineBuilder[F, M, prep.Out] =
    ...
    def +?[T1 <: HList](query: TypedQuery[F, T1])(implicit
        prep: Prepend[T1, T]): RequestLineBuilder[F, M, prep.Out] =
  ...
  case class PathCapture[F[_], M[_]](
      name: String,
